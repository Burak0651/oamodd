...
module.exports = {
  conf: {
    ...
    run: async (client, message, args, embed, author, prefix, guild) => {
        ...
        var member = message.mentions.member.first() || guild.member.cache.get(args[0]);
        ...
        guild.member.cache.get(member.id).roles.add(conf.registration.staff);
        guild.member.cache.get(member.id).roles.add(conf.registration.enaltyetkilirolü);
